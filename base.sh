...
ttl="900"                                  # Set the DNS TTL (seconds)
...
logger "DDNS Updater: Attempting to get public IP from Cloudflare trace"
if [[ ! $ret == 0 ]]; then
    logger "DDNS Updater: Failed to get IP from Cloudflare trace, trying other services"
...
logger "DDNS Updater: Retrieved IP - $ip"

# Use regex to check for proper IPv4 format.
if [[ ! $ip =~ ^$ipv4_regex$ ]]; then
    logger -s "DDNS Updater: Failed to find a valid IP. Retrieved IP: $ip"
    exit 2
fi
...
logger "DDNS Updater: Retrieved record - $record"
...
update=$(curl -s -X PATCH "https://api.cloudflare.com/client/v4/zones/${zone_identifier}/dns_records/$record_identifier" \
                     ...
                     -H "$auth_header ${auth_key}" \
                     -H "Content-Type: application/json" \
                     --data "{\"type\":\"A\",\"name\":\"${record_name}\",\"content\":\"$ip\",\"ttl\":900,\"proxied\":true}")

logger "DDNS Updater: Update response - $update"
...
*)
  ...
  if [[ $slackuri != "" ]]; then
    ...
    --data-raw '{
      "channel": "'$slackchannel'",
      "text" : "'${sitename}' Updated: ${record_name} new IP Address is $ip"
    }'
  fi
  if [[ $discorduri != "" ]]; then
    curl -i -H "Accept: application/json" -H "Content-Type:application/json" -X POST \
    --data-raw '{
      "content" : "'${sitename}' Updated: ${record_name} new IP Address is $ip"
    }' $discorduri
  fi
  exit 0;;
esac